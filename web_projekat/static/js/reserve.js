...
`, components : { 
	...
	created (){
		...
		axios
		.get('/apartment/datesForDisable/' + this.$route.query.id)
		.then(response => {
			let datess = [];
		...
		axios
		.get('/apartment/rangesForDisable/' + this.$route.query.id)
		.then(response => {
			let ranges = [];
			...
			for(let d of response.data)
				datess.push(new Date(d));
			this.disabledDates["dates"]= datess;
			//ranges.push({from : new Date(2020,9,25), to : new Date(2020,9,30)})
			*/
